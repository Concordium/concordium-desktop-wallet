...
export default class EmulatorTransport implements Transport {
    ...
    async send(
        ...
        try {
            ...
            let message;
            const messagePrefix = Buffer.concat([
                Buffer.of(cla),
                Buffer.of(ins),
                Buffer.of(p1),
                Buffer.of(p2),
            ...
            if (data) {
                ...
                message = Buffer.concat([
                    messagePrefix,
                    Buffer.of(dataAsNodeBuffer.length),
                    dataAsNodeBuffer,
                ]).toString('hex');
            } else {
                message = Buffer.concat([messagePrefix, Buffer.of(0)]).toString(
                    'hex'
                );
            }
            ...
            const emulatorResponse: AxiosResponse<LedgerEmulatorResponse> = await speculosEmulator.post(
                '/apdu',
                { data: message }
            );
